...
const LabExecPolicySchema = z.object({
  ...
  shell: z.string().describe("Shell execution policy. This primitive never uses a shell for allowed commands."),
...
export const LabExecResultSchema = z.object({
  ok: z.boolean().describe("True when the lab exec request was handled, including policy rejections."),
  ...
  cwd_label: z.string().describe("The child cwd label. The absolute local path is not returned."),
  exit_code: z.number().int().describe("Process exit code, or -1 when no process exit code is available."),
  signal: z.string().describe("Process signal when available, otherwise empty."),
