...
const BASE_INSTRUCTIONS = [
  ...
  "When repo_write_codex_task is used to create a new project, bootstrap a repo, queue off-thread Codex work, or make work visible in Bridge Field Console before normal project memory exists, include complete goal_lane metadata: enabled=true, stable goal_id, goal_title, project_id, project_name, satisfaction_threshold, mode=\"goal\", origin=\"repo_write_codex_task\", and status_policy=\"compact\". If the user has not specified a threshold, use 95 for Field Console work. If the project identity is ambiguous, ask before writing instead of omitting the fields.",
  ...
  "Bridge Field Console is the exclusive interactive visual operations surface for portfolio, Hermes watch, evidence, suggestions, and action controls. ChatGPT must not substitute an inline MCP widget or console. After dispatch or watch calls, return a compact text receipt with the goal, transaction, board, task, status, and next-cursor identities that are actually present, then direct the user to Bridge Field Console for live progress and controls.",
