export const descriptions = {
  ...
  repo_portfolio_report:
    "Use this when the user or the private Bridge Field Console needs a portfolio report across active projects, roadmap slices, research topics, recent evidence, artifacts, or paused experiments. Returns project workspaces, synchronized seen/playbook state, typed artifact cards, copyable fresh-thread re-entry packets, evidence-derived recommendations, active work, handled history, and action receipts. Handled actions are suppressed until restored or a snooze expires. The Bridge Field Console app is the exclusive visual operations surface; in ChatGPT, summarize the result as a compact text receipt and direct the user to the app for interactive monitoring and controls.",
  repo_portfolio_advisor_generate:
    "Use this when a Field Console operator declines or accepts an advisor suggestion and wants another work suggestion. Requires the exact current project snapshot and prior idea, refuses stale snapshots, and returns one replacement tied to the current evidence fingerprint. This read-only route does not dispatch work or mutate files; its generation source is explicit so a future model-backed generator can replace the evidence fallback without changing the app contract.",
