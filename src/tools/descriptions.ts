export const descriptions = {
  ...
  repo_hermes_watch:
    "Use this when the user asks ChatGPT or the private Bridge Field Console to watch, supervise, live-tail, or stay with Hermes Kanban or one Hermes off-thread transaction. This is the dedicated read-only resident watch: it polls Hermes-specific durable evidence server-side for up to 55 seconds, returns immediately on new evidence or a terminal state, and otherwise returns a factual heartbeat at the deadline. Continue normal watches with next_cursor; do not search unrelated sources. The Bridge Field Console app is the exclusive visual operations surface; ChatGPT should return a compact text observation or landing receipt, not an inline console. Never launches work, mutates files, overrides acceptance, stages, commits, pushes, deletes, or runs arbitrary shell commands.",
